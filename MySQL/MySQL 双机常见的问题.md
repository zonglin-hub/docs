...
- A 如果出现：
    ...
    ![image](./imgs/1023536612.png)

没有添加同步信息，双机没有组合成功。

- B 如果出现：

    ![image](./imgs/531338629.png)
...
​`Slave:Error "Duplicate entry '1' for key 1" on query.....`​可能是 master 未向
slave 同步成功，但 slave 中已经有了记录。造成的冲突可以在从库上执行 `set global
sql_slave_skip_counter=n;`​（n代表同步不成功数据条数）跳过几步。再 `restart
slave`​ 就可以了。
