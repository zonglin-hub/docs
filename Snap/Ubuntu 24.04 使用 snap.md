...
![snapcraft-1.avif](./imgs/snapcraft-1.avif)

Ubuntu Snap 是 Canonical 推出的软件包管理系统，旨在简化软件安装和管理。与传统
`apt` 不同，Snap 将软件及其依赖打包成一个独立的容器（称为 **Snap 包**），支持跨
发行版和自动更新。
...
在 Snap 包管理系统中，**删除某个 Snap 软件时，其依赖关系会一并被清理**。这是因为
Snap 的设计理念是将应用及其**所有依赖**打包成一个独立、自包含的容器。
...
使用 snap 安装的软件，它的 `.desktop` 文件存放在系统目录：
**`/var/lib/snapd/desktop/applications/`** 

在这个目录下，文件的命名格式通常是 **`<snap名称>_<应用名称>.desktop`** 。

### 💡 重要注意事项

关于这个文件，有两点很关键：

1.  **不要直接编辑原文件**：这个目录下的文件是由 snap 系统自动管理的。每当对应的
    snap 应用更新时，这个 `.desktop` 文件就会被覆盖重置。所以，你在这里做的任何
    修改都会丢失 。
2.  **如何自定义？**：如果你确实需要修改某个 snap 应用的启动命令、图标或其他属
    性，正确的做法是**覆盖它**。
    *   首先，将目标文件从上述目录复制到你的个人应用目
        录：`~/.local/share/applications/` 。
    *   然后，**务必保持文件名完全一致**，再对你复制的这个副本进行编辑。系统会优
        先使用你个人目录下的这个文件 。

另外，如果你想更方便地在文件管理器中访问这些快捷方式，也可以为这个目录创建一个软链接：
...
这样，你就能在 `~/.local/share/applications/snap` 里直接看到所有 snap 应用的
`.desktop` 文件了 。s