...
免」使用它。更多人倾向于使用 Fedora 的 Flatpak 打包系统。

## 安装 Flatpak

要在 **Ubuntu 18.10 （Cosmic Cuttlefish） 或更高版本**上安装 Flatpak，请打开终端
应用程序并运行：
...
自 Ubuntu 20.04 起，默认的软件中心是 Snap Store，并不支持 Flatpak 集成。因此，安
装下面的软件包将导致同时存在两个软件中心：一个是 Snap，另一个是 DEB。
...
> [!NOTE]
> **注意：** Ubuntu 20.04 ~ 23.04 版本中将 GNOME 软件作为 Snap 分发，并在 23.10
> 及更高版本中将其替换为 App Center ，两者都不支持安装 Flatpak 应用程序。安装
> Flatpak 插件还将安装 GNOME 软件的 deb 版本，从而在 Ubuntu 20.04 到 23.04 上同
> 时安装两个“软件”应用程序，并在 Ubuntu 23.10 及更高版本上安装一个新的“软件”应用
> 程序。
...
由 上海交通大学 Linux 用户组 (SJTUG) 维护 flathub 的镜像源。官方教
程：<https://mirror.sjtu.edu.cn/docs/flathub>
...
> [!NOTE]
> Flathub 镜像是 flathub.org 的智能缓存。当您请求镜像中的资源时， 如果文件没有被
> 镜像服务器缓存，我们会将您重定向回原站，并在后台进行缓存。也就是说，如果你要安
> 装的软件没有在上交镜像里缓存，那就会重定向到官方的flathub。如果没有代理的话，
> 可能会显示这个：
...
除了搜索 Flatpak 软件包之外，在其他情况下，`<package-name>` 指的是 Flatpak 软件
包名称，例如 `com.brave.Browser`（上述截图中的「应用程序 ID」）。您也可以使用
「应用程序 ID」中的关键单词，如 `brave`。
...
在一些特殊的情况下，可以直接从开发者的软件仓库安装 Flatpak 软件包。在这种情况
下，可以使用以下命令：
...
有时候，您可能会收到应用程序的 `.flatpakref` 文件，该文件不是离线安装包，但包含
了获取软件包所需的必要详细信息。
...
通常情况下，我们会在 Ubuntu 系统菜单中打开应用程序，但也可以使用以下命令从「终
端」运行 Flatpak 应用：
...
在 Linux 桌面中，通过命令行搜索 Flatpak 软件包肯定没有图形界面方便直观。我们可以
通过 [Flathub 网站](https://flathub.org/zh-Hans)浏览和查找 Flatpak 应用程序，该
网站提供了更多的详细信息，如验证的发布者、总下载次数等。还可以在应用程序页面底部
获取它所需的命令。
...
ls /var/lib/flatpak/app/
```
这两个路径分别是 Flatpak 应用的**系统级安装目录**和**用户级安装目录**，用于存储
已安装的 Flatpak 应用及相关数据。

### 1. `~/.local/share/flatpak/`

- **作用**：这是当前用户的**个人 Flatpak 应用目录**，所有安装在该目录下的应用仅
  对当前用户可见，无需超级用户权限。
- **包含内容**：通常包含以下子目录：
  - `app/`：存放用户级安装的应用（例如 `org.mozilla.firefox/` 等）。
  - `runtime/`：存放应用依赖的运行时环境。
  - `repo/`：Flatpak 使用的 OSTree 本地仓库，存储应用和运行时的对象数据。
  - `overrides/`：用户自定义的应用覆盖配置。
- **适用场景**：用户希望只为自己的账户安装应用，或没有系统管理员权限时使用。

### 2. `/var/lib/flatpak/app/`

- **作用**：这是**系统级 Flatpak 应用目录**，所有用户共享该目录下安装的应用，通
  常需要 `root` 权限才能修改（安装、更新、删除）。
- **包含内容**：`/var/lib/flatpak/` 的结构与用户级类似，其中：
  - `app/`：存放系统级安装的应用（每个应用对应一个子目录，如
    `org.gnome.gedit/`）。
  - 其他子目录如 `runtime/`、`repo/` 等也存在于该根目录下。
- **适用场景**：管理员为系统所有用户统一安装应用，节省磁盘空间并集中管理。

### 总结

- 两个目录本质上是相同结构的 Flatpak 存储位置，只是作用范围不同：系统级影响所有
  用户，用户级仅限当前用户。
- 当运行 `flatpak list` 或通过 GUI 启动应用时，Flatpak 会合并这两个位置的已安装
  应用，并优先使用用户级覆盖系统级的同名应用（如果存在）。

你可以通过以下命令分别查看两个级别的已安装应用：
```bash
# 查看系统级应用
flatpak list --system
# 查看用户级应用
