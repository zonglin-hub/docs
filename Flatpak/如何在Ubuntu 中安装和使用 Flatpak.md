...
ls /var/lib/flatpak/app/
```
这两个路径分别是 Flatpak 应用的**系统级安装目录**和**用户级安装目录**，用于存储已安装的 Flatpak 应用及相关数据。

### 1. `~/.local/share/flatpak/`
- **作用**：这是当前用户的**个人 Flatpak 应用目录**，所有安装在该目录下的应用仅对当前用户可见，无需超级用户权限。
- **包含内容**：通常包含以下子目录：
...
- **作用**：这是**系统级 Flatpak 应用目录**，所有用户共享该目录下安装的应用，通常需要 `root` 权限才能修改（安装、更新、删除）。
- **包含内容**：`/var/lib/flatpak/` 的结构与用户级类似，其中：
  - `app/`：存放系统级安装的应用（每个应用对应一个子目录，如 `org.gnome.gedit/`）。
  - 其他子目录如 `runtime/`、`repo/` 等也存在于该根目录下。
- **适用场景**：管理员为系统所有用户统一安装应用，节省磁盘空间并集中管理。

### 总结
- 两个目录本质上是相同结构的 Flatpak 存储位置，只是作用范围不同：系统级影响所有用户，用户级仅限当前用户。
- 当运行 `flatpak list` 或通过 GUI 启动应用时，Flatpak 会合并这两个位置的已安装应用，并优先使用用户级覆盖系统级的同名应用（如果存在）。
