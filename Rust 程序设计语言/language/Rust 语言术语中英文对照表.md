...
- P
    ...
    - preorder
        - 前序
    - panic
        - `不译` ；此单词直接翻译是“恐慌”，在 Rust 中用于不可恢复的错误处理
    - parameter
        - 参量，参数，形参，形式参量（数）；不严格区分的话， argument（参数）和  parameter（参量）可以互换地使用
    ...
    - phantom type
        - 虚类型，虚位类型 [^3]
            ...
            [^3]: phantom 相关的专有名词：phantom bug 幻影指令phantom power
